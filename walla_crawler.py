...
counterRSS = 1
counterNOTEXT = 1
print("*************this is walla site************")
while True:
    for rss_name in rss_name_dict.keys():
        try:
            # request RSS from dict
            rss_selected = requests.get(rss_name,
                                        proxies=proxyDict,
                                        headers=headers)
        ...
        if len(soup_rss_selected.findAll("item")) != 0:
            ...
            for link_index in range(1, len(soup_rss_selected.findAll("item")) + 1):
                ...
                if now_news_date_changed > last_Date_old_df_changed:
                    ...
                    try:
                        des = \
                            soup_rss_selected.findAll("item")[-link_index].find("description").contents[0].split(
                                "<br/>")[1]
                        des = re.sub('[<=-_>,;%/~"]', '', des)
                        des = re.sub('\n', '', des).strip()
                    ...
                    try:
                        text = soup_selected.findAll("section", attrs={"class": "article-content"})[0].text
                        text = re.sub('[~"]', '', text)
                        text = re.sub('\n', '', text).strip()
